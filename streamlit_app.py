...
with st.expander("這跟現成的資產配置工具差在哪"):
    st.markdown(
        """
台灣市面上有數個資產配置工具，**沒有一個幫你算出配置**——
Portfolio Visualizer 原版不收台股代號；YP 指投網、元大的計算器都要你
自己填好比例，它再告訴你這樣配過去表現如何。

**填比例**和**算出該填多少**是兩件事。這裡做的是後者。

最能說明問題的是：功能最完整的那個工具，頁面底部連結到一篇叫
〈想知道最佳的資產配置，效率前緣告訴你答案〉的**文章**——
使用者想要什麼很清楚，只是工具裡沒做。

跨市場還有兩個現成工具沒處理的問題：美股報酬要換成台幣才能跟台股相加；
台北比紐約早收盤十三個小時，日資料會把跨市場相關性從 +0.111 壓成 +0.010。
第二點會實質改變最佳化的答案（見「為什麼用週報酬」分頁）。

競品資訊為 2026 年 9 月實地檢視。
"""
    )
